...
5. The Brain validates the Event, runs Gemini multimodal verification (health events only), generates a voice script, calls ElevenLabs with streaming TTS, plays audio chunks through the glasses speaker in real time via Pygame, and writes an Event_Record to MongoDB Atlas — all synchronously before returning HTTP 200.
...
| `services/elevenlabs.py` | ElevenLabs streaming TTS client; `eleven_flash_v2_5` model, chunk-based audio delivery |
| `services/audio.py` | Pygame mixer; device selection, real-time chunk playback, fallback logic |
...
async def lifespan(app: FastAPI):
    ...
    elevenlabs_client = ElevenLabs()   # initialise ElevenLabs SDK client
...
4. ElevenLabs streaming synthesis (if voice_script non-empty, 15s timeout)
   → call client.text_to_speech.convert() with model_id="eleven_flash_v2_5"
   → returns an iterator of audio chunks (~75ms to first chunk)
   → on failure: log error, skip playback, continue

5. Pygame audio playback (stream chunks directly to glasses speaker)
   → select GLASSES_AUDIO_DEVICE or fall back to default
   → feed each chunk to Pygame mixer as it arrives (no temp file)
   → block until all chunks played
...
The Brain uses the `eleven_flash_v2_5` model for ~75ms time-to-first-audio. Audio chunks are fed directly to Pygame as they arrive — no temp file is written or deleted.
...
def synthesize_and_play(voice_script: str, voice_id: str, device: str) -> None:
    """
    Stream TTS audio from ElevenLabs and play it chunk-by-chunk via Pygame.
    Falls back to default audio device if the glasses speaker is unavailable.
    """
    client = ElevenLabs()
    ...
    # Collect chunks into a buffer for Pygame (Pygame requires seekable stream)
    buffer = io.BytesIO()
...
**Why `eleven_flash_v2_5`:** Lowest-latency multilingual model at ~75ms to first audio chunk. Appropriate for real-time patient alerts where responsiveness matters.

**No temp files:** The previous design saved an MP3 to `audio/{event_id}.mp3` and deleted it after playback. The streaming approach eliminates this entirely — audio bytes flow directly into an in-memory buffer, removing file I/O and cleanup complexity.

The Brain must extract a boolean from free-text Gemini responses. The parsing strategy is:

```python
def parse_gemini_verified(response_text: str) -> bool:
    """
    Returns True if the response indicates YES/confirmed.
    Looks for YES as the first word or in the first sentence.
...
To prevent flooding the Brain with duplicate events for the same ongoing activity:
...
class CooldownTracker:
    def __init__(self):
        ...
        self._cooldowns = {
            "face_recognized": 3.0,   # seconds per person name
            "eating":          5.0,
            "drinking":        5.0,
            "medicine_taken":  10.0,
        }
...
| ElevenLabs streaming failure | Brain | Log error, skip audio playback, continue to MongoDB |
| ElevenLabs stream timeout (>15s) | Brain | Treat as failure above |
...
The Brain never returns 5xx for downstream integration failures. The only 5xx response is HTTP 500 for unhandled exceptions, caught by a global FastAPI exception handler:
...
All external API calls use `asyncio.wait_for` with the configured timeout:
...
### Brain Settings (Pydantic-Settings)
...
If any required field is missing, Pydantic raises a `ValidationError` at startup with a descriptive message identifying every missing variable. The Brain catches this, logs it, and exits with code 1.
...
class VisionSettings(BaseSettings):
    ...
    BRAIN_PORT: int = 8000
    GEMINI_API_KEY: str
    VIDEO_SOURCE: int | str = 0   # 0 = default webcam; path for scrcpy mirror
...
# Monitor for unexpected subprocess exits
while True:
    for p in processes:
...
Property-based testing is applicable to AuraGuard AI because the system contains significant pure logic: JSON schema validation, Gemini response parsing, voice script generation, Event_Record transformation, and API response contract enforcement. These functions have clear input/output behavior and universal properties that hold across a wide input space. The `hypothesis` library (Python) is used for all property-based tests, configured to run a minimum of 100 iterations per property.

### Property 1: Valid Events Are Accepted, Invalid Events Are Rejected

*For any* Event payload, if it contains all required fields with their correct types and valid values, the Brain SHALL return HTTP 200; if it is missing any required field or contains a field with an invalid type, the Brain SHALL return HTTP 422.

**Validates: Requirements 1.2, 1.4, 16.2**

### Property 2: HTTP 200 Response Always Contains Echoed event_id

*For any* valid Event payload submitted to `POST /event`, the Brain's HTTP 200 response SHALL contain an `event_id` field whose value is identical to the `event_id` in the submitted payload.

**Validates: Requirements 1.5, 8.2**

### Property 3: Identity Events Always Set verified=true Without Calling Gemini

*For any* Event with `type="identity"`, the Brain SHALL set `verified=true` in the resulting Event_Record and SHALL NOT invoke the Gemini API.

**Validates: Requirements 2.4**

### Property 4: Health Event Gemini Prompts Are Subtype-Specific

*For any* health Event with a given `subtype`, the Gemini prompt constructed by the Brain SHALL contain a targeted question specific to that subtype (e.g., a prompt for `drinking` asks about beverages, not medication).

**Validates: Requirements 2.2**

### Property 5: Gemini Response Parsing Is Deterministic

*For any* Gemini response string that begins with "YES" (case-insensitive), `parse_gemini_verified` SHALL return `True`; for any response string that does not begin with "YES", it SHALL return `False`.

**Validates: Requirements 2.3**

### Property 6: Unverified Health Events Produce Empty Voice Scripts and No Audio

*For any* health Event where `verified=False` (due to Gemini returning NO or failing), the Brain SHALL produce an empty `voice_script`, SHALL NOT call the ElevenLabs API, and SHALL NOT attempt audio playback.

**Validates: Requirements 3.3, 4.1**

### Property 7: Identity Voice Scripts Contain All Person Profile Fields

*For any* identity Event with a `person_profile` containing `name`, `relationship`, `background`, and `last_conversation`, the generated `voice_script` SHALL contain all four of those values as substrings.

**Validates: Requirements 3.1**

### Property 8: Health Voice Scripts Contain Patient Name and Are Subtype-Appropriate

*For any* verified health Event with a given `subtype`, the generated `voice_script` SHALL contain the `PATIENT_NAME` value and SHALL reference the activity described by the `subtype`.

**Validates: Requirements 3.2, 3.4**

### Property 9: ElevenLabs Streaming Produces Audio Without Temp Files

*For any* Event that triggers ElevenLabs synthesis, the Brain SHALL stream audio chunks directly to Pygame without writing any file to disk. No `audio/{event_id}.mp3` file SHALL exist on the filesystem before, during, or after playback.

**Validates: Requirements 4.1, 5.1**

### Property 10: Event_Record Excludes image_b64 and Includes All Enrichment Fields

*For any* Event payload containing an `image_b64` field, the Event_Record written to MongoDB SHALL NOT contain an `image_b64` field, and SHALL contain all four enrichment fields: `verified`, `voice_script`, `processing_status`, and `processed_at`.

**Validates: Requirements 6.2, 6.6, 16.5**

### Property 11: Downstream Failures Never Produce HTTP 5xx

*For any* combination of downstream failures (Gemini failure, ElevenLabs failure, MongoDB failure, Pygame failure) during Event processing, the Brain SHALL return HTTP 200 (not HTTP 4xx or 5xx) to the Vision Engine.

**Validates: Requirements 8.3, 8.4, 20.2, 20.3, 20.4**

### Property 12: All Brain Responses Have Content-Type application/json

*For any* request to `POST /event` or `GET /health` — whether the request is valid, invalid, or causes an internal error — the Brain's response SHALL include the header `Content-Type: application/json`.

**Validates: Requirements 8.1**

### Property 13: Vision Engine Events Conform to JSON Contract

*For any* detection result (face recognition match or health item detection), the Event constructed by the Vision Engine SHALL contain all required JSON Contract fields (`event_id`, `timestamp`, `patient_id`, `type`, `subtype`, `confidence`, `image_b64`, `metadata`, `source`) with their correct types and value constraints.

**Validates: Requirements 13.1, 16.1**

### Property 14: Round-Trip Event Compatibility

*For any* valid Event payload produced by the Vision Engine's event builder, the Brain's Pydantic `Event` model SHALL be able to parse it without raising a `ValidationError`.
...
Unit tests cover specific examples, edge cases, and error conditions. Property-based tests (using `hypothesis`) verify universal properties across all inputs. Both are necessary for comprehensive coverage.

### Property-Based Tests

All property tests use `hypothesis` with `@settings(max_examples=100)`. Each test is tagged with a comment referencing its design property.

**Feature: aura-guard-ai**

| Property | Test Module | Key Generators |
|---|---|---|
| Property 1: Valid/invalid event acceptance | `tests/brain/test_event_validation.py` | `st.builds(Event, ...)`, random field removal/type corruption |
| Property 2: event_id echoed in response | `tests/brain/test_event_response.py` | `st.builds(Event, ...)` with mocked downstream |
| Property 3: Identity events skip Gemini | `tests/brain/test_gemini_routing.py` | `st.builds(Event, type=st.just("identity"), ...)` |
| Property 4: Subtype-specific Gemini prompts | `tests/brain/test_gemini_prompts.py` | `st.sampled_from(["eating", "drinking", "medicine_taken"])` |
| Property 5: Gemini response parsing | `tests/brain/test_gemini_parsing.py` | `st.text()` with YES/NO prefix strategies |
| Property 6: Unverified health → no audio | `tests/brain/test_voice_routing.py` | `st.builds(Event, type=st.just("health"), ...)` |
| Property 7: Identity voice script completeness | `tests/brain/test_voice_scripts.py` | `st.builds(PersonProfile, ...)` |
| Property 8: Health voice script content | `tests/brain/test_voice_scripts.py` | `st.sampled_from(subtypes)`, `st.text()` for patient name |
| Property 9: Streaming produces no temp files | `tests/brain/test_audio.py` | `st.builds(Event, ...)` with mocked ElevenLabs stream, assert no file written |
| Property 10: Event_Record transformation | `tests/brain/test_event_record.py` | `st.builds(Event, ...)` |
| Property 11: No 5xx on downstream failure | `tests/brain/test_degradation.py` | `st.builds(Event, ...)` with injected failures |
| Property 12: Content-Type header | `tests/brain/test_api_contract.py` | Various request types |
| Property 13: Vision Engine event conformance | `tests/vision/test_event_builder.py` | `st.builds(DetectionResult, ...)` |
| Property 14: Round-trip compatibility | `tests/integration/test_contract.py` | `st.builds(Event, ...)` |

### Unit Tests

- **Brain**: Specific examples for each voice script template, timeout behavior (mock delays), Pygame device fallback logic, ElevenLabs streaming chunk delivery, MongoDB partial_failure response, health endpoint with mocked Motor ping.
- **Vision Engine**: Face recognition pipeline with known fixture images, health detection with mocked Gemini responses, cooldown tracker behavior, event dispatch with mocked HTTP.
- **Caregiver Portal**: MongoDB connection failure displays cached data, Snowflake failure shows placeholder, severity color mapping.

### Integration Tests

- End-to-end: Vision Engine → Brain → MongoDB with all real services (requires `.env` with valid credentials).
- Brain + mocked Gemini/ElevenLabs: verifies full pipeline with controlled external responses.
- Caregiver Portal + MongoDB: verifies event feed renders correctly from real Event_Records.
...
```python
# Feature: aura-guard-ai, Property 1: Valid events accepted, invalid events rejected
@given(event=valid_event_strategy())
@settings(max_examples=100)
def test_valid_event_accepted(client, event):
