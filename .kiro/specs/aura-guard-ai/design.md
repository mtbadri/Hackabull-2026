...
*For any* Event that triggers ElevenLabs synthesis, the saved MP3 file path SHALL be `audio/{event_id}.mp3` where `event_id` matches the Event's `event_id` field exactly.
...
| Property 9: Audio file naming | `tests/brain/test_audio.py` | `st.uuids()` for event_id |
...
- **Brain**: Specific examples for each voice script template, timeout behavior (mock delays), Pygame device fallback logic, MongoDB partial_failure response, health endpoint with mocked Motor ping.
